...
def get_models(eln_params_dict):
    ...
    model_l = lm.ElasticNet(max_iter=10000)
    model_l.set_params(**eln_params_dict['l'])

    model_m = lm.ElasticNet(max_iter=10000)
...
def mc_simulation(N=10000, n_MC=10):
    ...
    for j in range(n_MC):
        ...
        model_l, model_m = get_models(opt_params_eln)
        ate_estimates[j, 0], se_estimates[j,0], CIs[j, :], mses[j, :] = dml(y_data, d_data, x_data_quad, model_l, model_m)
...
with open('results_eln.pkl', 'wb') as pickle_file:
